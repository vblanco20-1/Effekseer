...
vec4 _main(PS_Input Input)
{
    ...
    soft = clamp(soft, 0.0, _88.g_filterParams.z);
    soft = (soft * soft) * _88.g_filterParams.w;
    float contribution = max(soft, brightness - _88.g_filterParams.x);
    contribution /= max(brightness, 9.9999997473787516355514526367188e-06);
    return vec4((color * contribution) * _88.g_intensity.x, 1.0);
}
